...
-   **Na coleta de dados**: medindo a variável resposta em vários sítios focais em uma região (Fig.1a);

-   **Na análise multiescala**: medindo a característica da paisagem de interesse em múltiplas extensões espaciais ao redor de cada sítio focal (Fig. 1b);
...
Para esse exercício, vamos utilizar os dados de [Bueno e Peres (2018)](https://onlinelibrary.wiley.com/doi/full/10.1111/jbi.13499?casa_token=DusuIn3clAIAAAAA%3A_dadze5I-rbSaO2VnbIWrl_MzLBRJ69oznsdkiwXJOGO7vdQJriBEErCcdLDN-9G5xMatTuBQUBAbyLbhg). Escolhi esse estudo por que os autores calcularam a cobertura florestal em **diferentes escalas espaciais** e identificaram a **escala de efeito**. Isso nos permite comparar nossa abordagem com a do estudo, garantindo que estamos calculando corretamente.
...
# https://github.com/Ivana-Cardoso/Ivana-Cardoso.github.io/raw/main/posts/Escala_efeito/bueno_2018_insect_birds.csv
birds <- read.csv("bueno_2018_insect_birds.csv", row.names = 1)
birds[1:6, 1:3] # mostra as primeiras seis linhas e três colunas da tabela
```

Observe o formato dos dados da comunidade:

-   **Linhas**: Representam os **sítios** (ou local de amostragem). Cada linha corresponde a um sítio específico.
...
-   **Valores nas células**: O número em cada célula indica a **abundância** de uma determinada espécie em um sítio específico.
...
-   **Linhas**: Cada linha representa um **sítio**.

-   **Colunas**: Cada coluna contém uma **característica** específica do sítio, como coordenadas geográficas e quantidade de cobertura florestal.

Estamos interessados nas colunas que representam a cobertura florestal em diferentes escalas espaciais. Por exemplo, a coluna *forest.cover.50* (coluna 4) mostra a porcentagem de cobertura florestal dentro de um buffer de 50 metros ao redor de cada sítio. Outras colunas, como forest.cover.100 ou forest.cover.200, seguem a mesma lógica, mas para buffers maiores (100 m, 200 m, etc.).

Para esse exercício estamos interessados em descobrir qual a escala do efeito que melhor explica a relação entre riqueza de espécies de aves insetívoras e cobertura florestal. Por isso, o primeiro passo é transformar os dados de abundância em dados de riqueza:

```{r}
# Calcula a riqueza de especies de aves insetivoras em cada sitio
islands$richness = rowSums(ifelse(birds > 0, 1, 0))
```

### Determinando a escala do efeito usando correlação

No estudo de [Bueno e Peres (2018)](https://onlinelibrary.wiley.com/doi/full/10.1111/jbi.13499?casa_token=DusuIn3clAIAAAAA%3A_dadze5I-rbSaO2VnbIWrl_MzLBRJ69oznsdkiwXJOGO7vdQJriBEErCcdLDN-9G5xMatTuBQUBAbyLbhg), as variáveis preditora (cobertura florestal) e resposta (número de espécies) são log transformadas (log10 + 1) para as análises, isso porque se trata de um estudo de relação espécie-área, analisada através de modelos lineares simples. Por isso, **atente-se a como você está usando suas variáveis nas análises e certifique-se de incluí-las de maneira correta quando for calcular a escala do efeito**. Para calcular a escala do efeito usando correlação utilizaremos o seguinte formato de comando:

*nome do objeto \<- cor.test(variável preditora de interesse, variável resposta de interesse)*

Por exemplo, para calcular a correlação entre a cobertura florestal em escalas de 50 metros e a riqueza de espécies:
...
### Determinando a escala do efeito usando regressão linear

Como discutimos anteriormente, existem várias maneiras de determinar a escala do efeito. Uma delas é a correlação, já apresentada, e outra é a regressão linear. A seguir, veremos como determinar a escala do efeito utilizando regressão linear. Para isso, usaremos o seguinte formato de comando:

*nome do objeto \<- lm(variável resposta de interesse \~ variável preditora de interesse)*

Assim, nosso loop for ficará da seguinte forma:

```{r}
# Criando um vetor com as escalas desejadas (de 50 a 2000, de 50 em 50)
escalas <- seq(50, 2000, by = 50)

# Criando um data frame vazio para armazenar os resultados
resultados <- data.frame(Escala = numeric(), R2 = numeric(), Pvalor = numeric())

# Loop para calcular a regressao em cada escala
for (escala in escalas) {
  # Criando o nome da coluna dinamicamente
  nome_coluna <- paste0("forest.cover.", escala)
  
  # Verificando se a coluna existe no dataframe
  if (nome_coluna %in% colnames(islands)) {
    # Ajustando a regressao linear
    modelo <- lm(log10(islands$richness+1) ~ log10(islands[[nome_coluna]]+1))
    
    # Extraindo R2 e p-valor
    r2 <- summary(modelo)$r.squared
    p_valor <- summary(modelo)$coefficients[2, 4]  # valor de p do coeficiente da variável explicativa
...
```{r}
plot2 <-
  ggplot(resultados, aes(x = Escala, y = R2)) +
  geom_line() +
  geom_point() +
  labs(x = "Tamanho da escala (m)",
       y = "r² da regressão linear entre riqueza e 
cobertura florestal") +
  theme_bw(base_size = 16) +
  theme(panel.grid = element_blank(),
        panel.border = element_rect(colour = "black"),
...
Utilizando regressão linear também é possível notar que a escala do efeito é 200 metros.

Espero que este post tenha esclarecido a importância da escala e como ela pode influenciar os resultados de pesquisas. Agradeço por me acompanhar até aqui e desejo sucesso em suas futuras análises! 
Até a próxima!
